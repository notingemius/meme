...
config.resolver.resolveRequest = (context, moduleName, platform) => {
	try {
		...
		// УВАГА (фікс білого екрану): пакет @expo-google-fonts/dev НЕ бандлить
		// шрифти, а тягне їх по мережі (https://fonts.gstatic.com/...). У preview
		// Expo Go це ок, але у standalone APK useFonts() ніколи не резолвиться
		// офлайн/при поганій мережі, а екрани з `if (!fontsLoaded) return null`
		// лишаються БІЛИМИ назавжди. Тому підміну на dev робимо ЛИШЕ поза
		// продакшеном. У PRODUCTION беремо реальний @expo-google-fonts/inter —
		// він бандлить .ttf локально (миттєво й офлайн).
		if (
