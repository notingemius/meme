...
export interface GameState {
	...
	deck: number[] // СПІЛЬНА колода (id мемів), з якої роздаються карти; вичерпується за гру
	discard: number[] // скинуті/зіграні карти — повертаються в колоду, коли draw-пул порожній
...
export function applyAction(prev: GameState, action: Action, rng: Rng = Math.random): ActionResult {
	...
	switch (action.type) {
		...
		case "submit": {
			...
			p.hand.splice(idx, 1)
			s.discard.push(action.memeCardId)
		...
		case "nextRound": {
			...
			for (const p of activePlayers(s)) {
				// Добір рівно 1 карти зі спільної колоди (поки колода/скид не порожні).
				const need = HAND_SIZE - p.hand.length
				if (need > 0) dealCards(s, p, 1, rng)
			}
			s.round++
			s.submissions = []
			s.votes = []
...
// Ініціалізація СПІЛЬНОЇ колоди на початку гри: усі меми, перемішані.
function initDeck(s: GameState, rng: Rng) {
	const all = MEME_CARDS.map((m) => m.id)
	shuffle(all, rng)
	s.deck = all
	s.discard = []
}

// Тягне рівно одну карту зі спільної колоди. Якщо колода порожня —
// перемішує скид назад у колоду. Якщо й скид порожній — повертає null.
function drawCard(s: GameState, rng: Rng): number | null {
	if (s.deck.length === 0) {
...
// Роздає гравцю до `count` карт зі спільної колоди (скільки вистачить).
function dealCards(s: GameState, player: Player, count: number, rng: Rng) {
	for (let i = 0; i < count; i++) {
		const card = drawCard(s, rng)
		if (card === null) break // колода вичерпана — гравець догравати наявними картами
...
function expectedSubmissions(s: GameState): number {
	// Рахуємо лише гравців, які МОЖУТЬ зіграти карту: активні, не суддя,
	// і або мають карти на руці, або вже зробили сабміт. Це не дає раунду
	// зависнути, якщо у когось порожня рука (рідкісний випадок при вичерпанні).
	const submitterIds = new Set(s.submissions.map((x) => x.playerId))
	let n = 0
	for (const p of s.players) {
		if (!p.isActive) continue
		if (s.mode === "judge" && s.currentJudgeId === p.id) continue
		if (p.hand.length > 0 || submitterIds.has(p.id)) n++
	}
	return n
}
...
function botStep(s: GameState, rng: Rng): boolean {
	...
	if (s.phase === "playing") {
		for (const bot of bots) {
			...
			if (bot.hand.length === 0) continue // колода вичерпана — пропускаємо
...
export function viewFor(s: GameState, playerId: string): ClientView {
	...
	return {
		...
		deckCount: s.deck.length + s.discard.length,
		seq: s.seq,
	}
}
