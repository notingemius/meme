...
// Встановлення:  npx expo install react-native-tcp-socket expo-network
//
// Обидва телефони мають бути в ОДНІЙ Wi-Fi-мережі (або хотспот телефону-хоста).
// Хост відкриває TCP-сервер; гравці підключаються за IP хоста.
// ============================================================================

import type { ClientLink, LinkConnection, ServerLink } from './link';
import { DEFAULT_PORT } from './protocol';

// Лінивий require, щоб не падати в Expo Go, де native-модуля немає.
function getTcp(): any {
  try {
    ...
    throw new Error(
      'Режим «Гра з другом» потребує зібраного APK (react-native-tcp-socket не працює в Expo Go).',
    );
  }
}
...
class TcpConnection implements LinkConnection {
  ...
  constructor(id: string, private socket: any) {
    ...
    socket.on('data', (data: string | { toString: (enc: string) => string }) => {
...
export class LanServerLink implements ServerLink {
  ...
  start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // 0.0.0.0 — слухати на всіх інтерфейсах (щоб інші телефони бачили + loopback).
...
// Визначити локальну IP-адресу хоста (для показу другу / ручного вводу).
// Використовує expo-network.
