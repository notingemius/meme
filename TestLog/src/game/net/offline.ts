...
export type OfflineHostOptions = { rng?: () => number; botTickMs?: number; turnTimeoutMs?: number; resultsDelayMs?: number }
...
export class OfflineHost {
	...
	private resultsRound = -1
	private resultsDelayMs: number
	...
	constructor(private server: ServerLink, opts: OfflineHostOptions = {}) {
		...
		this.resultsDelayMs = opts.resultsDelayMs ?? 0
	}
	...
	// Авто-перехід до наступного раунду: коли фаза стала "results", показуємо
	// переможний мем на resultsDelayMs, а потім автоматично шлемо nextRound
	// (від імені хоста гри). Без цього гра зависала після кожного раунду.
	private maybeScheduleNextRound() {
		if (this.state.phase === "results" && this.state.round !== this.resultsRound) {
			this.resultsRound = this.state.round
			if (this.nextRoundTimer) clearTimeout(this.nextRoundTimer)
			const advance = () => {
				if (this.state.phase !== "results") return
				const hostId = this.state.hostPlayerId
				if (!hostId) return
				const res = applyAction(this.state, { type: "nextRound", playerId: hostId })
				if (!res.error) {
					this.state = res.state
					this.broadcast()
				}
			}
			if (this.resultsDelayMs > 0) {
				this.nextRoundTimer = setTimeout(advance, this.resultsDelayMs)
			} else {
				advance()
			}
		}
